...
function AiTriagePreviewSkeleton() {
  return (
    <div className="space-y-4" aria-busy aria-label="Ładowanie podglądu analizy">
...
/** Hidden for now — restore from QuickActions by setting SHOW_AI_ASSISTANT. */
export function AiTriagePanel({ enabled }: { enabled: boolean }) {
  ...
  const canRunAi = useMemo(
    () => aiText.trim().length > 0 && !propertiesLoading && properties.length > 0,
    [aiText, propertiesLoading, properties.length],
  );
  ...
  async function handleProcessAi() {
    ...
    try {
      ...
      console.error("[AiTriagePanel] handleProcessAi:", e);
